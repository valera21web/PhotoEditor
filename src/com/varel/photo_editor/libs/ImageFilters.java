...
public class ImageFilters {
   ...
   public Bitmap applyRotateEffect(Bitmap src, float degree) {
   ...
   public Bitmap applyJoinImages(Bitmap firstImage, Bitmap secondImage) {
   // can add a 3rd parameter 'String loc' if you want to save the new image - left some code to do that at the bottom
      ...
      else if(firstImage == null) {                    return secondImage; }
      else if(secondImage == null) {                   return firstImage; }
      Bitmap cs = null;
    ...
    * @return
    */
   public Bitmap applySimpleBorder(Bitmap image, Context context, float widthBorder) {
      int width = image.getWidth();
      int height = image.getHeight();

      Bitmap imageWithBorder = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
      Canvas comboImage = new Canvas(imageWithBorder);
      comboImage.drawBitmap(image, 0f, 0f, null);

      int newWidth = (int) (width * widthBorder);
      int newWidthSmall = (int) (newWidth / 2);
      Bitmap border;

      border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_left_top), newWidth, newWidth, true);
      comboImage.drawBitmap(border, 0f, 0f, null);

      border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_right_top), newWidth, newWidth, true);
      comboImage.drawBitmap(border, (width - newWidth) , 0f, null);

      border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_left_bottom), newWidth, newWidth, true);
      comboImage.drawBitmap(border, 0f, (height - newWidth), null);

      border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_right_bottom), newWidth, newWidth, true);
      comboImage.drawBitmap(border, (width - newWidth), (height - newWidth), null);

      // top
      int end_point = (width - newWidth);
      for(int left = newWidth; left < end_point; left += 2) {
         border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_top_bottom), newWidthSmall, newWidthSmall, true);
         comboImage.drawBitmap(border, left, 0f, null);
      }

      // bottom
      int left;
      int top = (height - newWidthSmall);
      end_point = (width - newWidth);
      for(left = newWidth; left < end_point; left += 2) {
         border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_top_bottom), newWidthSmall, newWidthSmall, true);
         comboImage.drawBitmap(border, left, top, null);
      }

      // left
      end_point = (height - newWidth);
      for(top = newWidth; top < end_point; top += 2) {
         border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_left_right), newWidthSmall, newWidthSmall, true);
         comboImage.drawBitmap(border, 0f, top, null);
      }

      // right
      end_point = (height - newWidth);
      left = (width - newWidthSmall);
      for(top = newWidth; top < end_point; top += 2) {
         border = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_border_1_left_right), newWidthSmall, newWidthSmall, true);
         comboImage.drawBitmap(border, left, top, null);
      }
   ...
   public Bitmap applyGaussianBlurEffect(Bitmap src) {
      ...
      return applyGaussianBlurEffect(src, GaussianBlurConfig);
   }

   public Bitmap applyGaussianBlurEffect(Bitmap src, double[][] GaussianBlurConfig) {
      if(src == null) return null;
   ...
   public  Bitmap applyReflection(Bitmap source) {
      if(source == null) return null;
      ...
      Paint paint = new Paint();
      LinearGradient shader = new LinearGradient(0, source.getHeight(), 0,
